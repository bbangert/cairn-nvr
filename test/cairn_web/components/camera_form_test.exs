defmodule CairnWeb.CameraFormTest do
  ...
  describe "to_params/1" do
    ...
    test "a three-decimal threshold survives the cell it renders into" do
      ...
      assert params |> CameraForm.rows() |> Enum.at(0) |> Map.get("min_score") == "0.555"
    ...
    test "the default row has no per-label days: they are the camera's own" do
      ...
      assert params |> CameraForm.rows() |> Enum.at(0) |> Map.get("retention_days") == ""
    end

    test "a new camera starts with one default row and nothing else" do
      params = CameraForm.to_params(nil)

      assert params["id"] == ""
      assert CameraForm.rows(params) == [CameraForm.blank_row("default")]
    end
  end
  ...
  describe "to_settings/2" do
    ...
    test "a tier column with no cell filled is omitted, not emptied" do
      ...
      {:ok, settings} = CameraForm.to_settings(params)
    ...
    test "a non-numeric whole-number field is an error naming the field" do
      params = Map.put(CameraForm.to_params(nil), "pre_window_seconds", "five")
    ...
    test "duplicate and unnamed label rows are errors" do
      ...
      assert {:error, [~s(duplicate label "person")]} = CameraForm.to_settings(duplicated)

      unnamed =
        params_with_rows([%{"label" => "default"}, %{"label" => "", "min_score" => "0.6"}])

      assert {:error, ["a label row needs a label"]} = CameraForm.to_settings(unnamed)
    end

    test "an entirely blank row is dropped rather than refused" do
      params = params_with_rows([%{"label" => "default"}, %{"label" => ""}])

      assert {:ok, settings} = CameraForm.to_settings(params)
    ...
    test "a cell that is not a number is passed on for the loader to name" do
      ...
      assert {:ok, %{"min_score" => %{"default" => "high"}}} = CameraForm.to_settings(params)
    end

    test "extra ffmpeg args are stored split, the way the importer stores them" do
      params = Map.put(CameraForm.to_params(nil), "extra_ffmpeg_args", "-rtsp_transport tcp")
  ...
  describe "urls/2" do
    test "probes the composed URL, credential and all" do
      row = rich_row()
      params = Map.put(CameraForm.to_params(row), "password", "other")
  ...
  describe "excluded?/3" do
    test "a blank cell is excluded only when the column has rules and no default" do
      ...
      assert CameraForm.excluded?(rows, car, "track")
      # The default row carries a record rule, so a blank record inherits it.
      refute CameraForm.excluded?(rows, car, "record")
      assert CameraForm.inherited(rows, car, "record") == "0.70"
    end
  end
  ...
  defp render_form(params, opts) do
    params =
      ...
      |> Map.put(
        "labels",
        CameraForm.index_rows([
          CameraForm.blank_row("default"),
          CameraForm.blank_row("person")
        ])
      )

    blank_probe = %{state: :idle, chips: [], warning: false, error: nil}

    render_component(&CameraForm.camera_form/1,
      form: Phoenix.Component.to_form(params, as: :camera),
  ...
  defp params_with_rows(rows) do
    rows = Enum.map(rows, &Map.merge(CameraForm.blank_row(""), &1))

    nil
    |> CameraForm.to_params()
    |> Map.put("labels", CameraForm.index_rows(rows))
  end
end
