defmodule Cairn.CameraStatusTest do
  ...
  # of ids to keep. It prunes against the `known` membership frozen on the
  # `{:config_changed, diff}` it received — not the live snapshot — so a rapid
  # delete/re-create can't make it retain a stale row.
  test "a config change drops the cameras the snapshot no longer names", %{id: id} do
    CameraStatus.put(id, %{status: :running})
    # cam_a is named by the config, so the prune keeps it; reset the global row
    # this write dirties so a later synchronous suite does not read it :running.
    CameraStatus.set("cam_a", :running)
    on_exit(fn -> CameraStatus.set("cam_a", :unknown) end)
