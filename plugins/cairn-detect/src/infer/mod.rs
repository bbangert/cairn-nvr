...
// main.rs, decode.rs, hwdecode.rs, multiplex.rs and emit.rs exactly as it did
// when this was one file.
pub use geometry::{Fit, InputSize, Projection};
pub use labels::{Labels, ScoreFloors};
// `decode.rs` names these two only from its tests, and a binary crate has no
// consumer outside itself — so an unconditional re-export reads as dead to
// rustc. Gate it rather than blanket-allow unused imports here, which would
// also hide a genuinely dead one.
pub use profile::{
    InputSizeSource, InputSpec, Layout, ModelProfile, NmsSpec, OutputSpec, Outputs, Roles,
    ScoreComposition, Shapes,
};
#[cfg(test)]
pub use {encoding::TensorEncoding, geometry::ResizePolicy};

use catalog::{names, PROFILES};
// The four family constants are named only by this module's own tests and the
// golden fixtures; sniffing walks PROFILES rather than any of them by name.
#[cfg(test)]
use catalog::{RFDETR, YOLOV10, YOLOV8, YOLOX};
use geometry::{MAX_INPUT_DIM, MAX_INPUT_PIXELS};
use labels::check_label_count;
use profile::grid_anchors;
#[cfg(test)]
use profile::DEFAULT_NMS;
