...
`sparsetrack_parity.py` is a different kind of run: not accuracy scoring, but
an exact-match gate. It drives the vendored SparseTrack reference over a
sequence and writes the MOT prediction lines that
`Membrane.MOTTracker.SparseTrack` must reproduce **byte for byte** —
`test/membrane_mot_tracker/sparse_track_parity_test.exs` compares with no
tolerance.

```sh
.venv/bin/python sparsetrack_parity.py          # regenerate the committed fixtures
```

The committed fixture is a synthetic sequence generated by that script, not
MOT data: the obligations below forbid redistributing MOT17/MOT20, and a
gate has to live in the repo. Any real sequence can be checked by hand, and
should be after a change to the port:

```sh
.venv/bin/python sparsetrack_parity.py --seq data/mot17/train/MOT17-09-SDP \
  --out /tmp/MOT17-09-SDP.expected.txt
# from the repo root
mix cairn.mot.track tools/mot-bench/data/mot17/train/MOT17-09-SDP \
  --tracker sparsetrack --out /tmp/MOT17-09-SDP.txt
diff /tmp/MOT17-09-SDP.txt /tmp/MOT17-09-SDP.expected.txt
```

Beware: `mix cairn.mot.track` canonicalizes ids to first-seen ordinals and so
does the script, which is what makes the two files comparable at all.
