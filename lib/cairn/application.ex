defmodule Cairn.Application do
  ...
  def start(_type, _args) do
    ...
    children = [
      ...
      Cairn.Registry,
      # PubSub must start before every config-subscriber below — the two table
      # owners here, `Cairn.EventCheckpoint`, the `Cairn.PresenceSupervisor`
      # that holds `Cairn.PresenceCheckpoint`, and `Cairn.CameraReaper`. Each
      # subscribes in init/1 and prunes only on `{:config_changed, _}`, so a
      # PubSub restart that dropped its subscription would leave it silently no
      # longer pruning — a re-created camera could then inherit a stale overlay.
      # No supervision group binds them to PubSub: each survives a PubSub
      # restart on its own by monitoring the PubSub pid and re-subscribing on
      # its `:DOWN` (`Cairn.ConfigSubscription`).
